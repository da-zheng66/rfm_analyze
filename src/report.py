...
SCATTER_TOOLTIP_JS = """
const scatterTooltip = document.createElement('div');
scatterTooltip.className = 'scatter-tooltip';
document.body.appendChild(scatterTooltip);

document.querySelectorAll('circle[data-tip]').forEach((circle) => {
  circle.addEventListener('mousemove', (event) => {
    scatterTooltip.textContent = circle.dataset.tip;
    scatterTooltip.classList.add('visible');
    scatterTooltip.style.left = `${event.clientX + 12}px`;
    scatterTooltip.style.top = `${event.clientY + 12}px`;
  });
...
<style>{CSS}{SCATTER_TOOLTIP_CSS}</style>
</head>
<body>
<main class="report">
{self._render_hero()}
...
<script>{SCATTER_TOOLTIP_JS}</script>
</body>
</html>
"""
    ...
    def _render_scatter_svg(self) -> str:
        ...
        for _, row in sample.iterrows():
            ...
            tip = (
                ...
                f"R {int(row['recency_days'])} · "
                f"F {int(row['frequency'])} · "
                f"M {self._fmt_money(row['monetary'])}"
            )
            points.append(
                f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{radius:.1f}" fill="{color}" opacity="0.58" data-tip="{escape(tip)}"><title>{escape(tip)}</title></circle>'
            )
